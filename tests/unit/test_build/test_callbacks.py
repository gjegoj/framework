...
from src.build import build_callbacks, build_trainer
...
def test_a_declared_logger_reaches_the_trainer(monkeypatch: pytest.MonkeyPatch) -> None:
    clearml_stub(monkeypatch)
...
def test_the_architecture_tags_a_logger_that_can_be_tagged() -> None:
    """Passed after construction through a port, so a logger by ``_target_`` that cannot be tagged is left alone."""
    from tests.support.fakes import PageLogger

    trainer = build_trainer(
        paper_config(logger={"_target_": "tests.support.fakes.PageLogger"}), architecture="resnet18"
    )

    assert isinstance(trainer.logger, PageLogger)
    assert trainer.logger.tags == ["resnet18"]
...
def test_a_declared_dirpath_reaches_the_saver() -> None:
    ...
    Nothing in the composition root decides it, so this is the whole path from declaration to
