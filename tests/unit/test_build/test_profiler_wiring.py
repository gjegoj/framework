...
def test_where_the_report_goes_is_whatever_config_said() -> None:
    """Nothing in the composition root decides this — the preset writes `${run.directory}`, and
...
def test_the_shipped_preset_inherits_the_default_trainer_rather_than_restating_it() -> None:
    """A preset that copies its neighbour drifts from it: a copy pinning `precision: 32-true` beside a
    default training in `bf16-mixed` profiles a program nobody runs. Inherited, it may say only which profiler.
    """
