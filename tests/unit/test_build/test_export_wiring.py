...
from src.build import example_inputs, export_model
...
from tests.support.fakes import PoolingModel, VectorModel
...
def test_the_declared_formats_are_built_in_order() -> None:
    ...
    exporters = build_exporters(paper_config(export=[{"name": "torchscript", "atol": 0.01}]).export)
...
def test_no_export_section_builds_nothing() -> None:
    ...
    assert build_exporters(paper_config().export) == []
...
def test_two_targets_of_one_format_are_refused_by_name() -> None:
    ...
    with pytest.raises(ValueError, match="torchscript"):
        build_exporters(paper_config(export=[{"name": "torchscript"}, {"name": "torchscript", "atol": 0.1}]).export)


def test_an_unknown_format_is_refused_before_training() -> None:
    """Failing at build time beats failing an hour into a run, and the message lists what exists."""
    with pytest.raises(LookupError, match="torchscript"):
        build_exporters(paper_config(export=[{"name": "torchscrpt"}]).export)
...
def test_an_input_that_is_not_a_picture_is_refused_as_an_export_example_by_name() -> None:
    """``image_size`` and ``mean`` shape a picture; a vector or text input shaped the same way would trace a
    graph the model never sees, silently — so the input is named and the export refused."""
    inputs = {
        "image": {"column": "image"},
        "vector": {"column": "label", "loader": {"_target_": "tests.unit.data.test_build.VectorLoader"}},
    }
    config = paper_config(data=DATA | {"inputs": inputs})

    with pytest.raises(ValueError, match=r"'vector'.*none"):
        example_inputs(config, 1)
