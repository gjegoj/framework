...
def test_a_per_class_reading_about_one_class_logs_its_mean_and_its_leaf() -> None:
    """torchmetrics squeezes what ``compute`` returns (measured, 1.9.0): a one-class reading arrives
    as two 0-d tensors, rewritten into the artifact behind its constructor's back."""
    reading = PerClass(torch.tensor([0.5]), torch.tensor([1]))
    reading.values, reading.classes = torch.tensor(0.5), torch.tensor(1)
    logged: dict[str, float] = {}

    report_metric(
        "val/boxes/map_per_class",
        reading,
        scalar_log=lambda key, value: logged.__setitem__(key, float(value)),
        loggers=[],
        step=0,
    ...
    assert logged == {
        log_keys.join("val/boxes/map_per_class", log_keys.MEAN): 0.5,
        "val/boxes/map_per_class/dog": 0.5,
    }
