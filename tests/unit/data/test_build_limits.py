...
from src.data.build import build_splitter
from tests.support.configs import INPUTS, SPLIT, paper_config, pipeline_of
...
def test_one_source_is_capped_before_the_split(tmp_path: Path) -> None:
    ...
    assert stage_sizes(pipeline_of(config)) == {Stage.TRAIN: 10, Stage.VAL: 5, Stage.TEST: 5}
...
def test_per_stage_sources_are_capped_one_stage_at_a_time(tmp_path: Path) -> None:
    ...
    assert stage_sizes(pipeline_of(config)) == {Stage.TRAIN: 20, Stage.VAL: 20, Stage.TEST: 20}
...
def test_a_share_caps_the_same_way(tmp_path: Path) -> None:
    ...
    assert sum(stage_sizes(pipeline_of(config)).values()) == 40
