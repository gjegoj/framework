...
from src.training.build import SCHEDULE_TITLE, build_scheduler_factory, fit_time_facts, per_group_rates
...
def scheduler_of(**declared: Any) -> SchedulerConfig | None:
    """The schedule under test, as the base experiment validates it."""
    return paper_config(scheduler=declared).scheduler
...
def test_a_step_sized_schedule_on_the_epoch_clock_is_refused() -> None:
    ...
    factory = build_scheduler_factory(scheduler_of(name="onecycle", max_lr=0.01))
...
def test_the_step_clock_sizes_the_schedule_from_the_fit() -> None:
    factory = build_scheduler_factory(scheduler_of(name="onecycle", max_lr=0.01, interval="step"))
    assert factory is not None
...
def test_a_plateau_without_a_monitor_is_refused_at_build_time() -> None:
    """Lightning would raise the same complaint at fit start, after data setup and model build."""
    with pytest.raises(ValueError, match="monitor"):
        build_scheduler_factory(scheduler_of(name="plateau"))


def test_a_plateau_with_a_monitor_reaches_lightning_with_it() -> None:
    factory = build_scheduler_factory(scheduler_of(name="plateau", monitor="val/loss", mode="min"))
    assert factory is not None
...
def test_a_rate_a_task_declared_survives_the_schedule_that_sets_rates_outright() -> None:
    """A scalar `max_lr` broadcasts, and the task's rate is gone.

    Measured: groups at 1e-3 and 1e-2 under `OneCycleLR(max_lr=1e-3)` both started at 4e-5 and both
    peaked at 1e-3, with nothing in the run saying so.
    """
    factory = build_scheduler_factory(scheduler_of(name="onecycle", max_lr=BASE_LR, interval="step"))
    assert factory is not None
...
def test_every_shipped_group_validates_and_builds(name: str) -> None:
    ...
    factory = build_scheduler_factory(scheduler_of(**declared))
    assert factory is not None
