...
class TestMosaic:
    ...
    @pytest.mark.parametrize("bad_ratio", [(0.7, 0.3), (0.0, 0.5), (0.3, 1.0)])
    def test_rejects_invalid_center_ratio(self, bad_ratio: tuple[float, float]) -> None:
        with pytest.raises(ValueError, match="center_ratio"):
            Mosaic(targets=[], center_ratio=bad_ratio)
...
class TestBatchTransformCallback:
    ...
    def test_rejects_invalid_fraction(self, bad_fraction: float) -> None:
        with pytest.raises(ValueError, match="disable_after_fraction"):
            BatchTransformCallback(_AddOneTransform(), disable_after_fraction=bad_fraction)
