"""Augmentations whose draw is the supervision: what they did to the picture becomes its target."""
...
def turned(before: np.ndarray, after: np.ndarray) -> int:
    """How many quarter-turns separate the two pictures — measured, not assumed."""
...
def test_the_turn_the_picture_took_is_what_its_target_says(seed: int) -> None:
...
@pytest.mark.parametrize("seed", SEEDS)
def test_the_picture_and_its_mask_take_the_same_turn(seed: int) -> None:
    before = picture()
...
def test_the_target_is_advanced_rather_than_set(seed: int) -> None:
    """It holds the picture's current turn, so an already-turned dataset stays truthful."""
    before = picture()
...
def test_a_turn_nobody_drew_leaves_the_target_alone() -> None:
    moved = bound(Rotate90(task="angle", p=0.0), seed=0)(Sample(inputs={"image": picture()}, targets={"angle": 2}))
...
def test_the_crop_says_so_in_the_target_it_names(seed: int) -> None:
    ...
    moved = bound(cropping, seed)(Sample(inputs={"image": picture()}, targets={"angle": 0}))
...
def test_a_crop_that_did_not_happen_is_the_other_class() -> None:
    ...
    moved = bound(cropping, seed=0)(Sample(inputs={"image": picture()}, targets={"angle": 0}))
...
def test_a_target_no_augmentation_answers_is_left_where_it_is() -> None:
    ...
    moved = bound(A.HorizontalFlip(p=1.0), seed=0)(Sample(inputs={"image": picture()}, targets={"angle": 2}))
...
def test_an_augmentation_answering_a_task_whose_target_is_pixels_is_refused() -> None:
    """A mask moves with the picture; an answer is written over. Mixing them up costs a silent epoch."""
