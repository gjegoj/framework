...
def task(name: str, topology: OutputTopology, objective: Objective = Objective.MULTICLASS) -> Task:
    return Task(name=name, output_topology=topology, objective=objective, metrics={})
...
def test_every_pixel_keeps_exactly_one_source() -> None:
    ...
    stitched = Mosaic([task("label", OutputTopology.GLOBAL)], profiling(label=3))(given)
...
def test_a_mask_is_swapped_by_the_same_seams_as_the_picture() -> None:
    ...
    tasks = [task("mask", OutputTopology.DENSE)]
...
def test_the_label_reports_the_share_the_picture_actually_shows() -> None:
    ...
    stitched = Mosaic([task("label", OutputTopology.GLOBAL)], profiling(label=4))(given)
    picture = stitched.inputs["image"][0, 0]
...
def test_a_continuous_target_is_weighted_as_the_number_it_already_is() -> None:
    ...
    stitched = Mosaic([task("price", OutputTopology.GLOBAL, Objective.CONTINUOUS)], DataProfile())(given)
...
def test_both_kinds_of_task_are_served_by_one_split() -> None:
    ...
    tasks = [task("label", OutputTopology.GLOBAL), task("mask", OutputTopology.DENSE)]
...
def test_the_batch_it_was_given_is_left_alone() -> None:
    ...
    Mosaic([task("label", OutputTopology.GLOBAL)], profiling(label=3))(given)
...
def test_a_metric_learning_task_is_refused() -> None:
    ...
    metric = task("pairs", OutputTopology.GLOBAL, Objective.METRIC)
...
def test_a_task_whose_targets_mosaic_cannot_rewrite_is_refused() -> None:
    ...
    with pytest.raises(ValueError, match="pairs"):
        Mosaic([task("pairs", OutputTopology.INSTANCES, Objective.MULTICLASS)], profiling(pairs=3))
...
def test_a_split_that_cannot_make_four_quadrants_is_refused(split_range: tuple[float, float]) -> None:
    with pytest.raises(ValueError, match="split_range"):
        Mosaic([task("label", OutputTopology.GLOBAL)], profiling(label=3), split_range=split_range)
