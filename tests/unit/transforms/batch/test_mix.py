...
from src.tasks import MetricLearning, Regression, Segmentation, Task, TaskKind
...
def task(name: str = "label", kind: TaskKind | None = None, classes: int | None = 3) -> Task:
...
def test_the_image_and_the_label_move_by_the_same_weight() -> None:
    ...
    mixed = MixUp(alpha=1.0).for_tasks([task()])(original)
...
def test_every_task_shares_one_mix() -> None:
    ...
    tasks = [task("label"), task("kind", classes=2)]
    given = batch()
    given.targets["kind"] = torch.arange(4) % 2

    mixed = MixUp().for_tasks(tasks)(given)
...
def test_cutmix_pastes_a_region_rather_than_blending() -> None:
    ...
    mixed = CutMix().for_tasks([task()])(given)
...
def test_cutmix_weights_the_label_by_the_area_it_actually_pasted() -> None:
    ...
    torch.manual_seed(0)
    mixed = CutMix().for_tasks([task()])(batch())
...
def test_a_continuous_target_is_mixed_as_a_number() -> None:
    ...
    mixed = MixUp().for_tasks([task("price", Regression(), classes=None)])(given)
...
def test_a_dense_task_is_refused() -> None:
    ...
    dense = a_task(name="mask", kind=Segmentation())

    with pytest.raises(ValueError, match="mask"):
        MixUp().for_tasks([dense])


def test_a_metric_learning_task_is_refused() -> None:
    """Proxy and margin losses break on soft labels."""
    metric = a_task(name="pairs", kind=MetricLearning())

    with pytest.raises(ValueError, match="pairs"):
        MixUp().for_tasks([metric])


def test_a_non_positive_alpha_is_refused() -> None:
    with pytest.raises(ValueError, match="alpha"):
        MixUp(alpha=0.0).for_tasks([task()])


def test_an_unbound_mix_refuses_a_batch_naming_the_binding() -> None:
    """Built from config without the tasks; the callback binds it when the trainer sets it up."""
    with pytest.raises(RuntimeError, match="for_tasks"):
        MixUp()(batch())
