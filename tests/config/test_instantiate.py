...
from src.config.instantiate import fill_signature, instantiate, resolve_target
from src.core import Registry
from tests.config.conftest import Widget


class TestResolveTarget:
    def test_a_name_resolves_through_the_registry(self, registry: Registry[Widget]) -> None:
        assert resolve_target(ComponentConfig.model_validate("widget"), registry) is Widget

    def test_an_import_path_resolves_without_a_registry(self) -> None:
        assert resolve_target(ComponentConfig.model_validate({"_target_": "tests.config.conftest.Widget"})) is Widget
    ...
    def test_a_name_without_a_registry_is_refused_naming_the_alternative(self) -> None:
        with pytest.raises(LookupError, match="_target_"):
            resolve_target(ComponentConfig.model_validate("widget"))

    @pytest.mark.parametrize(
        "target", ["tests.config.conftest.Missing", "no_such_module.Thing", "tests.config.conftest"]
    )
    def test_a_path_that_is_not_a_constructor_is_refused_by_name(self, target: str) -> None:
        with pytest.raises((LookupError, TypeError), match=target.rsplit(".", 1)[-1]):
            resolve_target(ComponentConfig.model_validate({"_target_": target}))
...
class TestInstantiate:
    ...
    def test_a_nested_import_path_is_built_first(self, registry: Registry[Widget]) -> None:
        declared = {"name": "widget", "inner": {"_target_": "tests.config.conftest.Widget", "tag": "inner"}}
