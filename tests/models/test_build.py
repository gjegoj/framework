...
from tests.models.conftest import MAP_WIDTH, POOLED_WIDTH
...
ENCODER = ComponentConfig(_target_="tests.models.conftest.Encoder")
...
def head(name: str = "linear", stream: str = Stream.POOLED, **params: Any) -> HeadConfig:
    return HeadConfig(name=name, input=stream, **params)
...
class TestSizing:
    ...
    def test_a_size_the_framework_derives_is_refused_where_it_was_restated(self, restated: str) -> None:
        declared = HeadConfig.model_validate({"name": "linear", "input": Stream.POOLED, restated: 3})
...
class TestFamilies:
    def test_a_model_that_arrives_whole_brings_its_own_heads(self) -> None:
        model = build_model(ModelConfig(_target_="tests.models.test_build.Whole"), {"t": head()}, {"t": CLASSES})
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                ModelConfig(_target_="tests.models.test_build.Whole", backbone=ENCODER),
                "brings its own",
                id="a whole model with a backbone",
            ),
            pytest.param(
                ModelConfig(name="composite", backbone=ComponentConfig(_target_="tests.models.test_build.Whole")),
                "not a Backbone",
