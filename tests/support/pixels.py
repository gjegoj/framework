...
def decoded(uri: str) -> np.ndarray:
    """The picture a browser would show, as RGB or RGBA — cv2 reads BGR, so the channels come back."""
    assert uri.startswith(PREFIX)
    raw = np.frombuffer(base64.b64decode(uri.removeprefix(PREFIX)), dtype=np.uint8)
    read = cv2.imdecode(raw, cv2.IMREAD_UNCHANGED)
    assert read is not None, "the bytes are not a picture any decoder will take"
