...
from src.build import build, load_checkpoint, run
...
"""Neither phase, for the tests that only want the built experiment."""
...
def test_a_distilled_runs_checkpoint_opens_a_plain_run(dataset_root: Path, tmp_path: Path) -> None:
    ...
    plain = build(distilling(dataset_root))
    load_checkpoint(plain.module.model, str(checkpoint))
