...
- **Config-driven** — ``disk_config(dataset_root, <the section this test is about>)``,
  then ``build`` and ``run``. Every one of these looks the same, and the only thing
...
- **Hand-built** — the objects built directly and Lightning driven by hand, for the
  runs whose subject is a wiring the composition root does not produce: a stand-in backbone, a
