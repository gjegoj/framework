"""Blending and pasting: two samples become one image, and their labels follow the same draw."""
...
by the same amount, or an image would take one neighbour's pixels and another neighbour's label.
"""


class LabelMix(ABC):
    """One draw mixes the image, and by the same weight every task's label.

    Every task is rewritten from the *same* draw, because the one image they all read changed once.
    Only tasks measured over the whole image are served: a mixed image has no coherent per-pixel
    target, and that is refused when the tasks are bound rather than at the first batch.

    What a subclass supplies is the mix itself — how the two images become one, and what share of
    ...
    Parameters:
        ...
        input_name: Which input holds the image.
    """
    ...
    def for_tasks(self, tasks: Sequence[Task]) -> Callable[[Batch], Batch]:
        ...
        if dense:
            raise ValueError(
                f"{type(self).__name__} makes one image out of two, and such an image has no coherent "
    ...
    def _applied(self, batch: Batch, tasks: Mapping[str, Task]) -> Batch:
        ...
        image, weight = self._mixed(require_tensor(batch.inputs[self._input_name], name=self._input_name))
        return Batch(
            inputs={**batch.inputs, self._input_name: image},
    ...
    @abstractmethod
    def _mixed(self, image: Tensor) -> tuple[Tensor, float]:
        """The image mixed with its neighbour, and the share of itself it kept."""


class MixUp(LabelMix):
    """Blend two images, and their labels, by one drawn weight."""

    def _mixed(self, image: Tensor) -> tuple[Tensor, float]:
        weight = self._drawn()
        return weight * image + (1.0 - weight) * image.roll(PAIRED_WITH, 0), weight


class CutMix(LabelMix):
    """Paste a patch of the neighbour over each image, and weigh the labels by what is left of it.
    ...
    describe an image nobody was shown.
    """
    ...
    def _mixed(self, image: Tensor) -> tuple[Tensor, float]:
        ...
        mixed = image.clone()
        mixed[..., top:bottom, left:right] = image.roll(PAIRED_WITH, 0)[..., top:bottom, left:right]
        return mixed, 1.0 - (bottom - top) * (right - left) / (height * width)

    def _patch(self, height: int, width: int) -> tuple[int, int, int, int]:
        """A box of the drawn area, around a point anywhere in the image and clipped at its edges."""
