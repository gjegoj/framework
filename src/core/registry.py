...
Being built does not un-register anything (a default may build registered
...
class Registry[T]:
    ...
    ``<package>/registry.py``, implementations register at import time, a declared name
    resolves through them. A registry holds what a declaration *names*; what it only
    *implies* (a composer built from a section's shape) has no name to register under, and
    ...
    def register_instance(self, key: Hashable, instance: T | None = None) -> Callable[[type[T]], type[T]] | None:
        ...
        For components configured once and shared. Pass the instance,
