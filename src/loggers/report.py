"""How a computed value reaches the logger that can draw it.

One router rather than a branch per caller: a metric\'s geometry — its type *and* its shape —
decides the route, and the ports it routes to live in ``loggers/ports.py``.
"""
...
from src.metrics.entities import Curve, Matrix, PerClass
...
def report_metric(
    ...
    *,
    ...
    Parameters:
        ...
        class_names (Sequence[str] | None): The task's class space, where it declared one.
    """
    match value:
        case Mapping():
        ...
        case tuple() | Tensor() if isinstance(value, tuple) or value.ndim >= 2:
            warnings.warn(
...
def _report_per_class(
    ...
    *,
    ...
    for index, value in reading.pairs():
        scalar_log(log_keys.join(key, _class_label(key, index, class_names)), value.float())


def _class_label(key: str, index: int, class_names: Sequence[str] | None) -> str:
...
def _completed_curve(key: str, curve: Curve, class_names: Sequence[str] | None) -> Curve:
...
def _completed_matrix(matrix: Matrix, class_names: Sequence[str] | None) -> Matrix:
...
def _aligned_names(key: str, size: int, class_names: Sequence[str] | None) -> Sequence[str]:
