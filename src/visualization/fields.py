"""One field, one input: what a single thing on a cell looks like in HTML."""
...
def leaves_of(label: Label) -> tuple[str, ...]:
    ...
    raise TypeError(
        f"No leaves for {type(label).__name__}; known: Classification, Classifications, Regression, Segmentation."
    )


@leaves_of.register
def _(label: Classification) -> tuple[str, ...]:
    return (label.label,)


@leaves_of.register
def _(label: Classifications) -> tuple[str, ...]:
    return tuple(item.label for item in label.classifications)


@leaves_of.register
def _(label: Regression) -> tuple[str, ...]:
    return ("value",)


@leaves_of.register
def _(label: Segmentation) -> tuple[str, ...]:
    return tuple(entry.name for entry in label.classes)


@singledispatch
def render_label(label: Label, context: FieldContext) -> list[FieldItem]:
    """One label → its overlays. Dispatch is by type: config never chooses here.

    ``html.py`` builds the page around what this returns and never asks what kind of
    thing it is drawing, which is the whole split between the two modules.
    """
    raise TypeError(
        f"No renderer for {type(label).__name__}; known: Classification, Classifications, Regression, Segmentation."
    )


@render_label.register
def _(label: Classification, context: FieldContext) -> list[FieldItem]:
    return [
        _chip(
            context,
            label.label,
            _said(label),
            context.colors.get(label.label, FALLBACK_COLOR),
            label.confidence,
        )
    ]
...
@render_label.register
def _(label: Classifications, context: FieldContext) -> list[FieldItem]:
    return [
...
@render_label.register
def _(label: Regression, context: FieldContext) -> list[FieldItem]:
...
@render_label.register
def _(label: Segmentation, context: FieldContext) -> list[FieldItem]:
...
def render_media(media: Media, alias: str, max_side: int | None = None) -> MediaItem:
    """One input, drawn. Dispatch by type, as labels are: a modality is not a config choice."""
    raise TypeError(f"No renderer for {type(media).__name__}; known: Image, Text.")


@render_media.register
def _(media: Image, alias: str, max_side: int | None = None) -> MediaItem:
...
@render_media.register
def _(media: Text, alias: str, max_side: int | None = None) -> MediaItem:
